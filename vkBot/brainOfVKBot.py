...
def start_message(event):
    # send_mail(event)
    bot.send_message(chat_id=event['object']['message']['peer_id'], text=messages.START)
    bot.send_message(chat_id=event['object']['message']['peer_id'], text=messages.REGISTRATION)
    bot.send_message(chat_id=event['object']['message']['peer_id'], text=messages.REGISTRATION_ASK_SURNAME)
...
def registration_ask_surname(event, information_about_student):
    ...
    bot.send_message(chat_id=event['object']['message']['peer_id'], text=messages.REGISTRATION_ASK_NAME)
    bot.register_next_step_handler(
        chat_id=event['object']['message']['peer_id'],
...
def registration_ask_name(event, information_about_student):
    ...
    bot.send_message(chat_id=event['object']['message']['peer_id'], text=messages.REGISTRATION_ASK_NUMBER_CLASS,
...
def registration_ask_number_class(event, information_about_student):
    ...
    bot.send_message(chat_id=event['object']['message']['peer_id'], text=messages.REGISTRATION_ASK_LETTER_CLASS,
...
def registration_ask_letter_class(event, information_about_student):
    ...
    bot.send_message(chat_id=event['object']['message']['peer_id'], text=messages.REGISTRATION_ASK_SUBCLASS,
...
def registration_ask_subclass(event, information_about_student):
    ...
    mediator.add_student(telegram_user_id=event['object']['message']['peer_id'], **information_about_student)
    welcoming(chat_id=event['object']['message']['peer_id'], information_about_student=information_about_student)
...
def command_settings(event):
    bot.register_next_step_handler(
        chat_id=bot.send_message(chat_id=event['object']['message']['peer_id'], text=messages.SETTINGS_INTRODUCING,
...
def settings(event):
    ...
    if text.lower() == 'параметры вывода':
        bot.send_message(chat_id=event['object']['message']['peer_id'], text=messages.SETTINGS_PARAMETERS_OUTPUT,
                         reply_markup=markups.get_parameters_output_inline_markup(
                             **mediator.get_parameters_output(telegram_user_id=event['object']['message']['peer_id'])
                         ))

    if text.lower() == 'в главное меню':
        bot.send_message(chat_id=event['object']['message']['peer_id'], text=messages.SETTINGS_INTRODUCING,
...
def ask_user_class(event):
    user_classes = mediator.get_user_classes(user_id=event['object']['message']['peer_id'])
    if len(user_classes) == 1:
        ask_needed_class(event, user_classes)
    else:
        classes_markup = markups.get_user_classes_markup(user_classes)
        bot.register_next_step_handler(
            chat_id=bot.send_message(chat_id=event['object']['message']['peer_id'], text=messages.SENDING_TIMETABLE_ASK_USER_CLASS,
...
def ask_needed_class(event, user_classes):
    ...
    bot.register_next_step_handler(
        chat_id=bot.send_message(chat_id=event['object']['message']['peer_id'], text=messages.SENDING_TIMETABLE_ASK_DATE,
                                 reply_markup=markups.choose_day_timetable),
        callback=send_timetable,
...
def send_timetable(event, needed_class):
    ...
    chat_id = bot.send_message(chat_id=event['object']['message']['peer_id'], text=text_timetable,
                               reply_markup=markups.menu)
    # bot.register_next_step_handler(
    #     chat_id=chat_id,
    #     callback=send_timetable,
